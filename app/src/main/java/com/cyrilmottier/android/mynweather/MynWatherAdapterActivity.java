...
public class MynWatherAdapterActivity extends ArrayAdapter<MynWeatherActivity> {
    ...
    public MynWatherAdapterActivity(Context context, List<MynWeatherActivity> forecast) {
        super(context, -1, forecast);
    }
    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        MynWeatherActivity day = getItem(position);
        ViewHolder viewHolder;
        if (convertView == null) {
            viewHolder = new ViewHolder();
            LayoutInflater inflater = LayoutInflater.from(getContext());
            convertView =
                    inflater.inflate(R.layout.list_activity, parent, false);
